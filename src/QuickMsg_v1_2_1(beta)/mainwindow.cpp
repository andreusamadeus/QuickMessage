...
//форма для просмотра пользователей он-лайн
QWidget * Form_online;
    ...
    else if (out[0]=="Message")
    {
        for (int i=0;i<(out.size()/2);i++)
        {
            ...
            else
            {
                ...
                text->document()->setHtml(text->document()->toHtml()+out[i*2+1]+":\t"+out[i*2+2]+"\n");
                text->setFocusPolicy(Qt::NoFocus);
...
void MainWindow::on_actionOnline_triggered()
{
    ...
    hbox = new QHBoxLayout(Form_online);
    list_online = new QListWidget(Form_online);

    hbox->addWidget(list_online);
